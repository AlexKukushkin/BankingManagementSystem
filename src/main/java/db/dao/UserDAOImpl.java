...
public class UserDAOImpl implements IUserDAO {
    ...
    public int getUserId(String login, String password) {
        ...
        try (Connection connection = manager.getConnection()){
            PreparedStatement statement = connection.prepareStatement("SELECT id_user FROM users WHERE login = ? AND password = ?");
            ...
            resultSet.next();
            index = resultSet.getInt("id_user");
        } catch (SQLException e) {
            logger.error("This is Error : " + e.getMessage());
        }
        return index;
    ...
    public Boolean createUser(User user) {
        ...
        try (Connection connection = manager.getConnection()) {
            PreparedStatement statement = connection.prepareStatement
                    ("INSERT INTO users (login, password, role) VALUES(?, ?, 'client')");
        ...
        try (Connection connection = manager.getConnection();
             PreparedStatement statement =
                     connection.prepareStatement(
                             "SELECT id_user, password, 'ROLE_' || upper(role::text) as role FROM users WHERE login = ?")) {
            statement.setString(1, login);
            ResultSet resultSet = statement.executeQuery();
